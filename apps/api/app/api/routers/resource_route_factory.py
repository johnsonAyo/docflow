...
from app.api.routers.resource_helpers import (
    create_resource,
    get_resource_stores,
    get_store,
    update_resource,
    ...
    *,
    ...
    async def create_endpoint(
        ...
        return create_resource(
            get_store(stores, collection), create_model.model_validate(payload)
        )

    router.add_api_route(
        path, list_endpoint, methods=["GET"], response_model=list[response_model]
    )
    router.add_api_route(
        path,
        create_endpoint,
...
def filter_params(request: Request, filters: tuple[str, ...]) -> dict[str, str]:
    return {
        key: request.query_params[key] for key in filters if key in request.query_params
    }
