...
def record_filters(
    workflow_id: str | None, document_run_id: str | None
) -> dict[str, str]:
    return {
        key: value
        for key, value in {
...
def record_rows(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for record in records:
        ...
        rows.extend(
            field_rows(record, fields) if fields else [empty_record_row(record)]
        )
    return rows
...
def empty_record_row(record: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "document_run_id": record["document_run_id"],
...
def field_rows(
    record: dict[str, Any], fields: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    return [
        {
            "record_id": record["id"],
...
def create_webhook_log(
    payload: WebhookTestPayload, stores: dict[str, ResourceStore]
) -> dict[str, Any]:
    ...
    stores["action_history"].create_item(
        {
            "entity_type": "integration_log",
            "entity_id": integration_log["id"],
