...
class TesseractOCRProvider:
    ...
    def extract_text(self, *, body: bytes, filename: str, content_type: str) -> OcrResult:
        ...
        return OcrResult(text="", issues=[{"field": "OCR", "message": f"Unsupported OCR content type: {content_type}"}], provider=self.name)
    ...
    def _extract_pdf(self, body: bytes) -> OcrResult:
        ...
        try:
            ...
            return OcrResult(text="", issues=[{"field": "OCR", "message": f"PDF page rendering failed: {exc}"}], provider=self.name)
        ...
        return OcrResult(text="\n\n".join(text_parts).strip(), pages=pages, issues=issues, provider=self.name)
    ...
    def _extract_image_path(self, image_path: Path, page_number: int) -> OcrResult:
        try:
            ...
            completed = subprocess.run([self.command, str(prepared_path), "stdout", "--psm", "6"], check=True, capture_output=True, text=True)
        except FileNotFoundError:
            return self._dependency_issue(f"Tesseract command not found: {self.command}")
        except subprocess.CalledProcessError as exc:
            return OcrResult(text="", issues=[{"field": "OCR", "message": f"Tesseract failed: {exc.stderr.strip() or exc}"}], provider=self.name)
        except Exception:
            return self._extract_image_path_without_preprocessing(image_path, page_number)
        return self._page_result(completed.stdout.strip(), page_number)

    def _extract_image_path_without_preprocessing(self, image_path: Path, page_number: int) -> OcrResult:
        completed = subprocess.run([self.command, str(image_path), "stdout", "--psm", "6"], check=True, capture_output=True, text=True)
        return self._page_result(completed.stdout.strip(), page_number)
    ...
    def _page_result(self, text: str, page_number: int) -> OcrResult:
        ...
        return OcrResult(text=text, pages=[OcrPage(page_number=page_number, text=text)], issues=issue, provider=self.name)

    def _dependency_issue(self, message: str) -> OcrResult:
        return OcrResult(text="", issues=[{"field": "OCR", "message": message}], provider=self.name)
