...
def money_match(text: str) -> str | None:
    match = re.search(
        r"(?:total|amount due)[:\s$]*([A-Z]{3}\s*)?\$?([0-9][0-9,]*(?:\.[0-9]{2})?)",
        text,
        re.IGNORECASE,
    )
    if match is None:
...
def date_match(text: str) -> str | None:
    match = re.search(
        r"(?:due|effective|date)[:\s]*([0-9]{4}-[0-9]{2}-[0-9]{2})", text, re.IGNORECASE
    )
...
def extract_invoice_fields(
    ...
    invoice_number = re.search(
        r"invoice(?: number)?[:\s#-]*([A-Z0-9-]+)", text, re.IGNORECASE
    )
    ...
    fields = [
        {
            "name": "Vendor",
            "value": vendor.group(1).strip() if vendor else "",
            "confidence": 0.9 if vendor else 0.0,
        },
        {
            "name": "Invoice number",
            "value": invoice_number.group(1).strip() if invoice_number else "",
            "confidence": 0.86 if invoice_number else 0.0,
        },
        {
            "name": "Due date",
            "value": due_date or "",
            "confidence": 0.84 if due_date else 0.0,
        },
        {
            "name": "Total amount",
            "value": total or "",
            "confidence": 0.82 if total else 0.0,
        },
    ]
...
def extract_contract_fields(
    ...
    fields = [
        ...
        {
            "name": "Effective date",
            "value": effective_date or "",
            "confidence": 0.84 if effective_date else 0.0,
        },
        {
            "name": "Renewal clause",
            "value": "Auto-renewal detected" if auto_renewal else "",
            "confidence": 0.73 if auto_renewal else 0.0,
        },
        {
            "name": "Payment terms",
            "value": payment_terms.group(1).strip() if payment_terms else "",
            "confidence": 0.8 if payment_terms else 0.0,
        },
    ]
...
def extract_fields(
    ...
    issues = (
        [] if text else [{"field": "Document text", "message": "No text extracted"}]
    )
