    ...
    review_state = review_states.create_item(
        {
            ...
            "status": "open",
            "issues": issues
            or [{"field": "OCR", "message": "No text could be extracted."}],
    ...
    *,
    ...
    try:
        return "ollama", OllamaExtractionProvider(
            ...
            document_type=document_type, text=text, schema_fields=schema_fields
        )
...
def store_record_result(
    *,
    ...
    text_artifact = document_store.put_object(
        artifact_key(
            f"processed/{context['workflow_id']}/{context['document_run_id']}/ocr",
            "ocr-text.txt",
        ),
        text.encode("utf-8"),
        "text/plain",
    ...
    review_state = review_states.create_item(
        review_document(context, issues, record["id"])
    )
    return {
        ...
        "processing": {
            ...
            "ocr_provider": context["ocr_provider"],
            "extraction_provider": context["extraction_provider"],
        },
    }


def record_document(
    context: dict[str, Any], issues: list[dict[str, Any]], text_artifact: dict[str, Any]
) -> dict[str, Any]:
    extraction = context["extraction"]
    return {
        "workflow_id": context["workflow_id"],
        "document_run_id": context["document_run_id"],
        "status": "needs_review"
        if issues or extraction.confidence < 0.82
        else "approved",
...
def review_document(
    context: dict[str, Any], issues: list[dict[str, Any]], record_id: str
) -> dict[str, Any]:
    return {
        "workflow_id": context["workflow_id"],
        "document_run_id": context["document_run_id"],
